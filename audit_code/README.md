## Documentation

- [Report example for FLAWS](https://docs.google.com/document/d/1fq0IjNVLRqy8ozJ7gCyUKrBvzNQbBVmIk1dRSQjiG6Y/edit)
- https://cwe.mitre.org/
- https://thedfirreport.com/
- https://github.com/pwndoc/pwndoc
- https://github.com/pwndoc-ng/pwndoc-ng
- https://owasp.org/www-community/attacks/

## Tools

- https://grep.app/
- https://www.first.org/cvss/calculator
- https://github.com/analysis-tools-dev/static-analysis
- https://github.com/analysis-tools-dev/dynamic-analysis


## Static Analysis

- https://github.com/semgrep/semgrep-rules/
- https://qwenlm.github.io/qwen-code-docs/en/
- https://github.com/microsoft/sarif-vscode-extension

### AI

- https://github.com/shuvonsec/claude-bug-bounty

#### MCP

- https://wilgibbs.com/blog/defcon-finals-mcp/
- https://github.com/FuzzingLabs/mcp-security-hub
- https://github.com/trailofbits/claude-code-config
- https://github.com/FlorianBruniaux/claude-code-ultimate-guide/blob/main/tools/audit-prompt.md

```bash
# https://www.cnblogs.com/alexander17/p/19089720
## check also ../reverse
cd ~/idapro*
pip install git+https://github.com/mrexodia/ida-pro-mcp
uv run ida-pro-mcp --install

claude plugin marketplace add mrexodia/claude-marketplace
claude plugin install ida-pro-mcp@mrexodia

# IDA -> CTRL+ALT+M -> http://127.0.0.1:13337/config.html
```

#### Skills

- https://github.com/Th4nat0s/Chall_Tools/blob/master/skills/security/SKILL.md

### Pentesting Applications (Whitebox)

- https://owasp.org/www-project-noir/ # extract apis
- https://github.com/elttam/semgrep-rules
- https://github.com/trailofbits/semgrep-rules	# Go, Js, Java, Python, Ruby, Swift + Devops Tools/Configs & Secrets (Docker,Ansible,Terraform)

#### Containers

- https://github.com/aquasecurity/trivy
- https://github.com/eliasgranderubio/dagda
- https://github.com/stealthcopter/deepce
- https://github.com/cdk-team/CDK

#### Language specific

- https://brakemanscanner.org/              # Ruby
- https://github.com/PyCQA/bandit           # Python
- https://github.com/phpstan/phpstan        # Php
- https://github.com/koalaman/shellcheck    # Bash

#### Leaked Credentials

- https://github.com/gitleaks/gitleaks
- https://github.com/trufflesecurity/trufflehog

### Reversing Binaries (Blackbox)

- https://decompilation.wiki/
- https://github.com/trailofbits/BTIGhidra
- https://github.com/semgrep/semgrep-rules/
- https://github.com/0xdea/semgrep-rules                # C/C++
- https://github.com/0xdea/weggli-patterns              # C/C++
- https://github.com/9hozt/wegglist			# C/C++ Wrapper 
- https://github.com/synacktiv/Weggli_rules_SSTIC2023   # C/C++, Autodocumented with bash script
- https://github.com/NASA-SW-VnV/ikos			# C/C++, compiler
- https://github.com/trailofbits/codeql-queries         # C/C++, Go, Java
- https://github.com/clearbluejar/pyghidra-mcp


## Dynamic Analysis

- https://github.com/bata24/gef
- https://github.com/google/sanitizers/wiki/
- https://valgrind.org/docs/manual/quick-start.html
- https://www.lazenca.net/display/TEC/03.Analysis


## Fuzzing

- https://github.com/secfigo/Awesome-Fuzzing
- https://www.lazenca.net/display/TEC/04.Fuzzing
- https://gitlab.com/opensecuritytraining/fuzz1001_intro_slides_and_subtitles
