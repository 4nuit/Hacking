...
What's replacing RSA/ECC for key exchange and signatures once a cryptographically-relevant quantum computer exists (Shor's algorithm breaks both integer factorization and discrete log, so all of RSA/DH/ECDH/ECDSA fall - see the [ECC](./asym/elliptic_curves) and RSA sections above). NIST finalized the first PQC standards in August 2024:
