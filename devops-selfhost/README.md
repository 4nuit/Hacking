...
Anti-pattern above: `--build-arg` bakes the value into the image's layer history (`docker history`/`docker inspect` recovers it even after removal from later layers). Non-secret args (`DEBUG`, `ALLOWED_HOSTS`) are fine this way; a real secret (`SECRET_KEY`) should be passed at **runtime**, not build time:

```bash
# runtime env (still visible to anyone with docker inspect access, but not baked into the shareable image)
docker run -e DJANGO_SECRET_KEY="$SECRET_KEY" -p 80:80 chatbox

# or, if it must be available at build time: BuildKit secret mount (never touches a layer)
# syntax=docker/dockerfile:1
# RUN --mount=type=secret,id=django_key ...
docker build --secret id=django_key,env=DJANGO_SECRET_KEY -t chatbox .
```
...
[Nextcloud in Docker](./containers/nextcloud/docker-compose.yml)
