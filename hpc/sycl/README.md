...
- USM allocations (`malloc_device`/`malloc_host`/`malloc_shared`) are plain, manually-managed pointers - there's no refcounting like `std::shared_ptr`. "shared" means the allocation is accessible from both host and device (migrated on page fault), not that its lifetime is automatic: you still own the `free(ptr, queue)` call. Fast, but becomes slower than buffer accessors with many task dependancies.
- [SYCL 101 task](https://www.intel.com/content/www/us/en/docs/sycl/introduction/latest/02-sycl-basic-code-single-task.html)
- [Khronos SYCL 2020 spec - Unified Shared Memory](https://registry.khronos.org/SYCL/specs/sycl-2020/html/sycl-2020.html#sec:usm)
