...
- [llama-cli.sh](./llama-cli.sh) / [llama-server.sh](./llama-server.sh) - local scripts, run a GGUF model (Qwen3.5-0.8B here) via [llama.cpp](https://github.com/ggml-org/llama.cpp)'s official Docker image (CLI vs. OpenAI-compatible HTTP server on :8080). Note: `llama-cli.sh`'s `[ ! -d "$~/Models" ]` check is broken (`$~` isn't a valid path expansion) - the model-download step re-runs unconditionally rather than being skipped once already downloaded.
