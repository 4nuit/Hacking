...
```bash
# routes towards wlan0 rather then eth0
curl --interface wlan0 -4 -Iv --connect-timeout 15 https://lscr.io/v2/

# metrics change default routing to wlan0
sudo ip route replace default via 10.138.130.226  dev wlan0 metric 50

# change back to eth0
sudo ip route replace default via 10.138.130.226 dev wlan0 metric 600
```
