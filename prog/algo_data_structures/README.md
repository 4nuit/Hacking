...
| **Delete (end)**      | `pop_back()` (O(1))                                      | `pop()` (O(1))            | `remove(size()-1)` (O(1) - no shift; `remove(index)` on any earlier index is O(n)) |
...
- Linked Lists (`C++: stack<T, list<T>>` - `std::stack` adapted over `std::list` instead of the default `std::deque`, `Java: LinkedList`)
...
**Search O(1) average (O(n) worst-case on collisions), key/value, no order/random access**

- `C++: Unordered map`
- `Python: Dict`
- `Java: HashMap` (`Hashtable` also exists but is a legacy, synchronized class predating the Collections framework - `HashMap`, or `ConcurrentHashMap` for concurrent access, is the idiomatic choice)
