...
# find/delete all hardlinks to a file
find ~ -inum $(ls -i file.txt  | awk '{print $1'}) -ls
find ~ -inum $(ls -i file.txt  | awk '{print $1'}) -delete
```
...
grep -rho 'https\?://[^"'"'"'<> ]*media\.agentless\.io[^"'"'"'<> ]*' ~/Hacking | sort -u | wget -i -
```

```bash
# decoding base64 lines
for e in $(seq 1 $(cat attack.txt |wc -l)); do echo $(sed -n $e"p" attack.txt) | base64 -d; done
```

```bash
# grepping through curl
curl -s 'https://jsonplaceholder.typicode.com/users/[1-11]' -o /dev/null -w "%{http_code}\n"
```
