...
- use `nullptr` over `NULL`/`0`: `NULL` is just `0` or `(void*)0`, an integer, so `f(NULL)` with overloads `f(int)`/`f(char*)` picks the wrong (`int`) one - `nullptr` has its own type (`std::nullptr_t`) that only converts to pointer types, so overload resolution can't get it wrong. See [cppreference - nullptr](https://en.cppreference.com/w/cpp/language/nullptr.html)
