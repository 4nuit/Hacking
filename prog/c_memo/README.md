## Documentation

- https://en.wikipedia.org/wiki/Pthreads
- https://en.wikipedia.org/wiki/Signal_(IPC)
- https://en.wikipedia.org/wiki/C_standard_library
- https://www.man7.org/linux/man-pages/man3/intro.3.html
- https://www.gnu.org/software/gnu-c-manual/gnu-c-manual.html
- https://www.open-std.org/jtc1/sc22/wg14/www/C89Rationale.pdf
- https://cmu-sei.github.io/secure-coding-standards/sei-cert-c-coding-standard/
- https://cyber.gouv.fr/publications/regles-de-programmation-pour-le-developpement-securise-de-logiciels-en-langage-c


## Courses

- [Beej's Guide to C Programming (Pointers, DSA, Memory, Multithreading)](https://beej.us/guide/bgc/html/split/index.html)
- [Beej's Guide to C Programming - Libray Reference](https://beej.us/guide/bgclr/html/split/)
- [What every programmer should know about C's undefined behaviours- LLVM](https://blog.llvm.org/2011/05/what-every-c-programmer-should-know.html)
- https://sites.uclouvain.be/SystInfo/notes/Theorie/html/index.html

### Tutos

- https://github.com/codr7/hacktical-c
- https://zestedesavoir.com/tutoriels/755/le-langage-c-1/
- https://beej.us/blog/data/function-pointers-cooperative-multitasking/

### FAQ

- https://c-faq.com/
- https://brennan.io/2015/01/16/write-a-shell-in-c/
- https://blog.abhi.host/blog/2010/04/15/very-simple-http-server-writen-in-c/

### Undefined behaviours

- https://ss64.com/
- https://wordsandbuttons.online/so_you_think_you_know_c.html
- https://paragonie.com/blog/2016/02/how-safely-store-password-in-2016
- [undefined_behaviours/](./undefined_behaviours) - local examples: integer promotion, alignment/pointer size, post-increment UB, misc "dark corners"

**Catching UB in practice** - [safe_c_compile.sh](./safe_c_compile.sh) enables ASan (`-fsanitize=address`, catches memory errors) but not UBSan; add `-fsanitize=undefined` to also catch the UB in the examples above (signed overflow, misaligned access, etc.) at runtime:

```bash
clang -Wall -Wextra -std=c99 -fsanitize=address,undefined -g -o main main.c && ./main
```

- `valgrind --tool=memcheck --leak-check=full ./main` - catches what ASan doesn't need a recompile for (e.g. use of uninitialized memory), at a bigger runtime cost

### Kernel

- https://539kernel.com/book/index.html # OS from scratch

#### Modules for drivers

- https://lwn.net/Kernel/LDD3/
- https://broux.developpez.com/articles/c/driver-c-linux/
- https://github.com/1mm0rt41PC/linux-driver-for-pci-io-card/

## Tools

- https://www.frama-c.com/
