...
- https://www.framaroot.net/ # one-click exploits target pre-Android 5/6 - non-functional on any current device, kept for historical/legacy-device reference only
- https://xdaforums.com/t/info-boot-process-android-vs-linux.3785254/
- https://xdaforums.com/t/guide-2025-how-to-root-a-phone-a-complete-truthful-up-to-date-crap-free-and-no-bs-guide.4710038/ # current method (Magisk/KernelSU-based)
...
- https://www.ragingrock.com/AndroidAppRE/ -> **ThaiCamera,FotaProvider,Mediacode corrected**
- https://www.evilsocket.net/2017/04/27/Android-Applications-Reversing-101/
...
Example of permissions of a malicious application
...
A malicious application can access this application's data and execute code:
...
### Android Studio (dynamic)

- https://developer.android.com/tools/adb
- https://developer.android.com/studio/intro
- https://github.com/android/ndk/wiki/unsupported-downloads#r9d # GDBserver - NDK r9d is ancient; NDK 24+ dropped GDB entirely in favor of LLDB, see NDK section below
...
```bash
# If the phone is off - running Thaicamera fails
...
Legacy (`gdbserver`, removed from NDK 24+):
...
Current (`lldb-server`, ships with NDK 24+ under `toolchains/llvm/prebuilt/<host>/lib/clang/<ver>/lib/linux/<arch>/lldb-server`):

```bash
adb push toolchains/llvm/prebuilt/linux-x86_64/lib/clang/*/lib/linux/aarch64/lldb-server /data/local/tmp/
adb shell "chmod 755 /data/local/tmp/lldb-server && /data/local/tmp/lldb-server platform --listen '*:5039' --server"
adb forward tcp:5039 tcp:5039
# then, on host: lldb -> platform select remote-android -> platform connect connect://localhost:5039
```
...
```bash
wget https://github.com/frida/frida/releases/download/16.1.8/frida-inject-16.1.8-android-x86_64.xz # choose the architecture based on the emulated phone
xz -d frida-inject*
```
...
Java.perform(function () {
        var main_activity = Java.use("com.example.<application/project>.<Main (without smali)>");
...
adb push frida-inject-16.1.8-android-x86_64 /data/local/tmp # depends on the architecture of the chosen phone
```

```bash
adb shell "ps -A | grep <application/project name>"
adb shell "/data/local/tmp/frida-inject* -p <PID obtained>   -s exploit.js"
```
